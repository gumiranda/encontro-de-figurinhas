...
const Icon = ({ name, size = 18, color = "currentColor" }) => {
  ...
  switch (name) {
    ...
    case "unlock": return <svg {...c}><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>;
    ...
    case "pin": return <svg {...c}><path d="M12 22s-8-7.58-8-13a8 8 0 0 1 16 0c0 5.42-8 13-8 13z"/></svg>;
    ...
    case "ai": return <svg {...c}><path d="M12 8V4H8"/><rect x="4" y="8" width="16" height="12" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><circle cx="9" cy="14" r="1" fill="currentColor"/><circle cx="15" cy="14" r="1" fill="currentColor"/></svg>;
    default: return null;
  }
};
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <svg viewBox="0 0 32 32" width={size} height={size}>
        ...
        {/* head */}
        <rect x="5" y="6" width="22" height="20" rx="4" fill={`url(#g-body-${size})`} stroke="var(--primary)" strokeWidth="0.7" />
        {/* screen face */}
        <rect x="7.5" y="9.5" width="17" height="11" rx="2.5" fill={`url(#g-screen-${size})`} stroke="var(--outline-variant)" strokeWidth="0.4" />
        {/* eyes */}
        {eyeShape === "round" && (
          <>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ whiteSpace: "nowrap" }}>{isLive ? "AO VIVO" : isFinished ? "ENCERRADO" : match.date.toUpperCase()}</span>
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div className="h-display" style={{ fontSize: 15, marginTop: 6, color: match.home.color }}>{match.home.code}</div>
          <div style={{ fontSize: 11, color: "var(--muted)" }}>{match.home.name}</div>
        </div>
        <div style={{ textAlign: "center", padding: "0 4px", whiteSpace: "nowrap" }}>
          {isFinished ? (
            <div className="h-display num-mono" style={{ fontSize: 36, letterSpacing: "0.05em" }}>
              {MATCH_RESULT.home}<span style={{ color: "var(--muted)", margin: "0 4px" }}>·</span>{MATCH_RESULT.away}
            </div>
          ) : isLive ? (
            <>
              <div style={{
                fontFamily: "var(--mono)", fontSize: 10, color: "var(--muted)", letterSpacing: "0.1em",
              }}>1º TEMPO</div>
              <div className="h-display" style={{ fontSize: 22, marginTop: 2 }}>32'</div>
            </>
          ) : (
            <>
              <div style={{
              ...
              <div className="h-display" style={{ fontSize: 20, marginTop: 2 }}>{timeToKickoff}</div>
            </>
          )}
        </div>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 44, lineHeight: 1 }}>{match.away.flag}</div>
          <div className="h-display" style={{ fontSize: 15, marginTop: 6, color: match.away.color }}>{match.away.code}</div>
...
function ScorePicker({ value, onChange, locked, color }) {
  ...
  return (
    <div style={{
      ...
      <button onClick={inc} disabled={locked}
        style={{
          width: 44, height: 28, padding: 0,
          background: "transparent", color: locked ? "var(--outline-variant)" : "var(--muted)",
      ...
      <div className="h-display num-mono" style={{
        fontSize: 56, lineHeight: 1, color, letterSpacing: "-0.04em",
        minWidth: 60, textAlign: "center",
      }}>{value}</div>
      <button onClick={dec} disabled={locked}
        style={{
          width: 44, height: 28, padding: 0,
          background: "transparent", color: locked ? "var(--outline-variant)" : "var(--muted)",
          border: "1px solid var(--outline-variant)", borderRadius: 8,
...
function YourPalpite({ match, state, palpite, setPalpite, onConfirm, confirmed }) {
  ...
  return (
    <div className="raised" style={{
      padding: "16px 14px",
      marginBottom: 14,
      background: confirmed ? "linear-gradient(180deg, rgba(149,170,255,0.08), var(--container))" : "var(--container)",
      border: `1px solid ${confirmed ? "var(--primary)" : "var(--outline-variant)"}`,
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div className="h-display" style={{ fontSize: 13, display: "inline-flex", alignItems: "center", gap: 6 }}>
        ...
        {confirmed && (
          <span style={{
            ...
            fontFamily: "var(--headline)", fontSize: 9, fontWeight: 700, letterSpacing: "0.12em",
          }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 12 }}>
        <ScorePicker value={palpite.home} onChange={(v) => setPalpite({ ...palpite, home: v })}
          locked={isLocked} color="var(--primary)" />
        <div className="h-display" style={{ fontSize: 28, color: "var(--muted)" }}>×</div>
        <ScorePicker value={palpite.away} onChange={(v) => setPalpite({ ...palpite, away: v })}
          locked={isLocked} color="var(--primary)" />
      </div>

      {!confirmed && state === "preMatch" && (
        <button onClick={onConfirm} className="btn btn-primary" style={{ width: "100%", marginTop: 14 }}>
...
function ConfidenceBar({ value }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span className="num-mono" style={{ fontSize: 12, color: "var(--tertiary)" }}>{value}%</span>
    </div>
  );
}
...
function GepetoCard({ match, state, userConfirmed }) {
  ...
  const mood = state === "postMatch" ? (gepetoCorrect(g) ? "smug" : "angry") : sealed ? "thinking" : "neutral";

  return (
    <div className="tcard fade-up" style={{
      padding: 0, marginBottom: 14,
      position: "relative", overflow: "hidden",
    }}>
      <div className="tcard-bg" style={{
        background: sealed
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            <span className="h-display" style={{ fontSize: 16, color: "var(--fg)" }}>Gepeto</span>
            <span style={{
              padding: "1px 6px", borderRadius: 4,
              background: "rgba(255,201,101,0.15)", color: "var(--tertiary)",
              fontFamily: "var(--headline)", fontSize: 8, fontWeight: 700, letterSpacing: "0.15em",
            }}>IA</span>
            <span style={{
              padding: "1px 6px", borderRadius: 4,
              background: "rgba(149,170,255,0.12)", color: "var(--primary)",
              fontFamily: "var(--headline)", fontSize: 8, fontWeight: 700, letterSpacing: "0.15em",
            }}>NV 7</span>
          </div>
          <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2, display: "inline-flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
        ...
        {!sealed && (
          <div style={{
            ...
            <div className="label-mono" style={{ fontSize: 8, marginTop: 2 }}>CONFIANÇA</div>
          </div>
        )}
        {revealed && (
          <div style={{
            padding: "4px 8px", borderRadius: 6,
            background: gepetoCorrect(g) ? "rgba(79,243,37,0.15)" : "rgba(255,110,132,0.15)",
            color: gepetoCorrect(g) ? "var(--secondary)" : "var(--error)",
            fontFamily: "var(--headline)", fontSize: 9, fontWeight: 700, letterSpacing: "0.12em",
            display: "inline-flex", alignItems: "center", gap: 4, flexShrink: 0,
          }}>
            {gepetoCorrect(g) ? <><Icon name="check" size={10} /> ACERTOU</> : <><Icon name="x" size={10} /> ERROU</>}
          </div>
        )}
      </div>
        ...
        {sealed ? (
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{ textAlign: "center" }}>
                <div className="h-display num-mono" style={{
                  fontSize: 48, lineHeight: 1, color: "var(--tertiary)",
                  letterSpacing: "-0.03em",
                }}>{g.homeGoals}</div>
                <div className="label-mono" style={{ fontSize: 8, marginTop: 4 }}>{match.home.code}</div>
              </div>
              <div className="h-display" style={{ fontSize: 22, color: "var(--muted)" }}>×</div>
              <div style={{ textAlign: "center" }}>
                <div className="h-display num-mono" style={{
                  fontSize: 48, lineHeight: 1, color: "var(--tertiary)",
                  letterSpacing: "-0.03em",
                }}>{g.awayGoals}</div>
                <div className="label-mono" style={{ fontSize: 8, marginTop: 4 }}>{match.away.code}</div>
              </div>
            </div>
            <div style={{ marginTop: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span className="label-mono" style={{ fontSize: 9 }}>NÍVEL DE CONFIANÇA</span>
                <span className="label-mono" style={{ fontSize: 9, color: g.confidence >= 75 ? "var(--tertiary)" : "var(--primary)" }}>
                  {g.confidence >= 80 ? "ALTÍSSIMA" : g.confidence >= 65 ? "ALTA" : g.confidence >= 50 ? "MÉDIA" : "BAIXA"}
                </span>
              </div>
      ...
      {!sealed && (
        <div style={{
          ...
          <span style={{
            fontFamily: "var(--headline)", fontSize: 26, lineHeight: 1,
            color: "var(--tertiary)", opacity: 0.5,
          }}>"</span>
          <div style={{ flex: 1, fontSize: 13, lineHeight: 1.4, color: "var(--fg)", fontStyle: "italic" }}>
            {g.voice}
          </div>
        </div>
      )}
        ...
        <div style={{ padding: "14px 16px 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            {peekable && (
              <span style={{
                display: "inline-flex", alignItems: "center", gap: 4,
                fontFamily: "var(--headline)", fontSize: 8, fontWeight: 700, letterSpacing: "0.12em",
                color: "var(--primary)",
              }}><Icon name="lock" size={9} /> RESUMO</span>
            )}
          </div>
            ...
            {g.insights.slice(0, peekable ? 2 : g.insights.length).map((i, idx) => (
              <div key={idx} style={{
                ...
                <div style={{
                  ...
                  background: i.weight === "strong" ? "rgba(255,201,101,0.18)" : "rgba(149,170,255,0.15)",
                  color: i.weight === "strong" ? "var(--tertiary)" : "var(--primary)",
                  display: "grid", placeItems: "center", flexShrink: 0,
                  marginTop: 1,
                }}>
                  <Icon name={i.weight === "strong" ? "lightning" : "sparkles"} size={10} />
                </div>
                <div style={{ fontSize: 12, color: "var(--fg)", lineHeight: 1.4 }}>{i.text}</div>
              </div>
            ))}
            {peekable && g.insights.length > 2 && (
              <div style={{
                padding: "8px 10px",
                background: "repeating-linear-gradient(45deg, rgba(149,170,255,0.04) 0, rgba(149,170,255,0.04) 8px, transparent 8px, transparent 16px)",
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, color: "var(--muted)", display: "inline-flex", gap: 5, alignItems: "center" }}>
          <Icon name="clock" size={11} />
          {sealed ? "Palpite blindado até o apito" : peekable ? "Análise abre quando o juiz apitar" : "Liberado"}
        </div>
        <button style={{
    ...
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, animation: "pop 0.5s ease-out" }}>
        <div style={{
          ...
          display: "grid", placeItems: "center",
          fontFamily: "var(--headline)", fontSize: 22, fontWeight: 700,
          boxShadow: "0 0 0 3px rgba(79,243,37,0.35)",
        }}>🏆</div>
      </div>
      <div style={{ position: "relative", zIndex: 1, flex: 1 }}>
        <div className="label-mono" style={{ fontSize: 9, color: "var(--secondary)" }}>BADGE DESBLOQUEADA</div>
        <div className="h-display" style={{ fontSize: 16, marginTop: 2 }}>{text}</div>
        <div style={{ fontSize: 11, color: "#bff5ad", marginTop: 2 }}>+10 pts · Streak 7🔥</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span className="label-mono" style={{ fontSize: 10, color: "var(--tertiary)" }}>PALPITE LACRADO</span>
      </div>
      <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 10, lineHeight: 1.4 }}>
        Gepeto já gravou o palpite dele.<br />
        Mande o seu primeiro pra ver o que ele cravou.
      </div>
      <div style={{
        fontFamily: "var(--mono)", fontSize: 9, color: "var(--outline)",
...
function VerdictBanner({ match, palpite }) {
  const userExact = palpite.home === MATCH_RESULT.home && palpite.away === MATCH_RESULT.away;
  const gepetoExact = gepetoCorrect(match.gepeto);
  const userWonner = !gepetoExact && userExact;
  // user got winner direction right
  const userWinner = (palpite.home > palpite.away) === (MATCH_RESULT.home > MATCH_RESULT.away);
  const userPts = userExact ? 25 : userWinner ? 10 : 0;
  const gepetoPts = gepetoExact ? 25 : 10;
  const userWon = userPts > gepetoPts;

  return (
    <div className="tcard fade-up" style={{ marginBottom: 14, padding: 0, overflow: "hidden" }}>
      <div className="tcard-bg" style={{
        background: userWon
      ...
      <div style={{ padding: "16px 16px 12px" }}>
        <div className="label-mono" style={{ color: userWon ? "var(--secondary)" : "var(--error)" }}>
        ...
        <div className="h-display" style={{
          ...
          {userWon ? "Mostrou pro robô como se palpita." : "Ele te pegou nessa, mas a volta tá engatilhada."}
        </div>
      </div>
      <div style={{
        ...
        <ScoreSlice label="VOCÊ" who="user" home={palpite.home} away={palpite.away}
          exact={userExact} winner={userWinner} pts={userPts} isWinner={userWon} />
        <ScoreSlice label="GEPETO" who="gepeto" home={match.gepeto.homeGoals} away={match.gepeto.awayGoals}
          exact={gepetoExact} winner={true} pts={gepetoPts} isWinner={!userWon} />
      </div>
      <div style={{
    ...
    <div style={{
      ...
      {who === "gepeto" ? (
        ...
        <div style={{
          ...
          margin: "0 auto 4px",
          display: "grid", placeItems: "center",
          fontFamily: "var(--headline)", fontSize: 13, fontWeight: 700,
        }}>MI</div>
      )}
      <div className="label-mono" style={{ fontSize: 9 }}>{label}</div>
      <div className="h-display num-mono" style={{
        fontSize: 24, marginTop: 4, letterSpacing: "0.02em",
      ...
      <div className="num-mono" style={{
        fontSize: 14, marginTop: 4, fontWeight: 700,
  ...
  return (
    <div className="raised" style={{ padding: 14, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
        <div className="h-display" style={{ fontSize: 13 }}>O que a galera acha</div>
        <span className="label-mono">12.4K PALPITES</span>
      </div>
      <div style={{
        display: "flex", height: 32, borderRadius: 10, overflow: "hidden",
      ...
      <div style={{
        ...
        Gepeto concorda com {community.home > 45 ? `${community.home}%` : "a minoria"} da galera nessa.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="raised fade-up" style={{ padding: 14, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <div className="num-mono" style={{ color: "var(--tertiary)", fontSize: 14, fontWeight: 700 }}>
          {streak}🔥
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${days.length}, 1fr)`, gap: 5 }}>
        {days.map((d, i) => (
          <div key={i} style={{
            aspectRatio: "1/1.2",
            borderRadius: 8,
            background: d.today ? "transparent"
              : d.beat ? "linear-gradient(160deg, rgba(79,243,37,0.25), rgba(255,201,101,0.18))"
              : d.on ? "var(--container-high)"
              : "transparent",
            border: d.today ? "1px dashed var(--primary)"
              : d.beat ? "1px solid var(--secondary)"
              : "1px solid var(--outline-variant)",
            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2,
            fontFamily: "var(--mono)", fontSize: 10,
            color: d.today ? "var(--primary)" : d.beat ? "var(--secondary)" : "var(--muted)",
            fontWeight: 700,
          }}>
            <span>{d.day}</span>
      ...
      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 10 }}>
        Palpita hoje pra manter sua sequência. Bata 7 e ganha o badge <b style={{ color: "var(--tertiary)" }}>Cassetete</b>.
      </div>
    </div>
  );
}
...
const WEEKLY = {
  ...
  communityScore: 9,
  totalMatches: 16,
  voice: "Semana 3 e os humanos continuam tropeçando no meu raciocínio. Cravar Brasil 2-1 Argentina não é palpite, é matemática. Quem discorda, abre o Excel e me chama na DM. Próxima rodada eu vou pesado: Espanha-Inglaterra é cilada e eu já anotei.",
  highlights: [
    {
      matchup: "BRA × ARG", g: "2-1", real: "2-1", gepetoGot: true, communityGot: false,
    ...
    {
      matchup: "FRA × NED", g: "1-0", real: "0-1", gepetoGot: false, communityGot: true,
    ...
    {
      matchup: "GER × MEX", g: "3-1", real: "3-1", gepetoGot: true, communityGot: false,
    ...
    {
      matchup: "POR × URU", g: "1-1", real: "2-1", gepetoGot: false, communityGot: false,
  ...
  return (
    <div style={{
      ...
      <button onClick={onToggle} style={{
        ...
        <div style={{ fontFamily: "var(--mono)", fontSize: 12, fontWeight: 600 }}>{h.matchup}</div>
        ...
        <div style={{
          ...
          background: h.communityGot ? "rgba(79,243,37,0.18)" : "rgba(255,110,132,0.15)",
      ...
      {expanded && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "flex-start" }}>
            ...
            <div style={{ flex: 1, fontSize: 12, color: "var(--fg)", lineHeight: 1.4, fontStyle: "italic" }}>
              “{h.gepetoVoice}”
            </div>
          </div>
          <div className="label-mono" style={{ fontSize: 9, marginBottom: 6, color: h.gepetoGot ? "var(--secondary)" : "var(--error)" }}>
          ...
          <div style={{ display: "grid", gap: 5 }}>
            {h.reasoning.map((r, i) => (
              <div key={i} style={{
                ...
                <span style={{ color: h.gepetoGot ? "var(--secondary)" : "var(--error)", flexShrink: 0, fontWeight: 700 }}>{h.gepetoGot ? "✓" : "✕"}</span>
                <span>{r}</span>
              </div>
            ))}
          </div>
...
function WeeklyNarrative() {
  ...
  return (
    <div style={{ padding: 16 }}>
      {/* Hero */}
      <div className="tcard fade-up" style={{ padding: 0, marginBottom: 14, overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div className="h-display num-mono" style={{
              fontSize: 38, marginTop: 6,
              color: ahead ? "var(--tertiary)" : "var(--fg)",
              letterSpacing: "-0.02em",
            }}>{w.gepetoScore}</div>
            <div className="label-mono" style={{ fontSize: 8, marginTop: 4 }}>GEPETO</div>
          </div>
          <div className="h-display" style={{ fontSize: 22, color: "var(--muted)" }}>×</div>
          <div style={{ textAlign: "center" }}>
            <div style={{
              width: 32, height: 32, borderRadius: 99,
              background: "var(--container-high)", color: "var(--primary)",
              margin: "0 auto",
              display: "grid", placeItems: "center",
            }}>
            ...
            <div className="h-display num-mono" style={{
              ...
              letterSpacing: "-0.02em",
            }}>{w.communityScore}</div>
            <div className="label-mono" style={{ fontSize: 8, marginTop: 4 }}>HUMANOS</div>
          </div>
        </div>
        ...
        <div style={{
          ...
          <span style={{
            fontFamily: "var(--headline)", fontSize: 32, lineHeight: 0,
            color: "var(--tertiary)", opacity: 0.45, marginRight: 4,
            verticalAlign: "-8px",
          }}>"</span>
          {w.voice}
        </div>
      ...
      <div className="raised fade-up" style={{ padding: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div className="h-display" style={{ fontSize: 13 }}>Resultados da semana</div>
          <span className="label-mono" style={{ fontSize: 9 }}>TOQUE PARA EXPANDIR</span>
        </div>
        <div style={{ display: "grid", gap: 6 }}>
        ...
        <div style={{
          fontSize: 10, color: "var(--muted)", marginTop: 10,
          display: "flex", gap: 14, justifyContent: "center",
        }}>
          <span><b style={{ color: "var(--secondary)" }}>G</b> palpite Gepeto</span>
          <span><b style={{ color: "var(--secondary)" }}>●</b> &gt;50% da galera</span>
        </div>
      </div>
          ...
          {w.topHumans.map((u, i) => (
            <div key={u.nick} style={{
              ...
              <div style={{
                ...
                color: "white",
                display: "grid", placeItems: "center",
                fontFamily: "var(--headline)", fontSize: 11, fontWeight: 700,
              }}>{u.nick.slice(0, 2).toUpperCase()}</div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700 }}>@{u.nick}</div>
                {u.isMe && <div className="label-mono" style={{ fontSize: 8, color: "var(--primary)" }}>VOCÊ</div>}
              </div>
              <div className="num-mono" style={{ fontSize: 16, fontWeight: 700, color: "var(--secondary)" }}>
                {u.score}
              </div>
            </div>
          ))}
        </div>
        <button className="btn btn-ghost" style={{ width: "100%", marginTop: 8, fontSize: 12, height: 36 }}>
...
const FIXTURES = [
    ...
    rounds: [
      {
        ...
        matches: [
          { h: "BRA", hf: "\ud83c\udde7\ud83c\uddf7", a: "CMR", af: "\ud83c\udde8\ud83c\uddf2", status: "final", score: [4, 1], g: "3-1", gOk: true, mine: "4-1", mineOk: true },
          { h: "ARG", hf: "\ud83c\udde6\ud83c\uddf7", a: "POL", af: "\ud83c\uddf5\ud83c\uddf1", status: "final", score: [2, 0], g: "1-0", gOk: false, mine: "2-0", mineOk: true },
        ],
      },
    ],
  },
  {
    ...
    rounds: [{
      ...
      matches: [
        { h: "BRA", hf: "\ud83c\udde7\ud83c\uddf7", a: "GHA", af: "\ud83c\uddec\ud83c\udded", status: "final", score: [3, 0], g: "2-0", gOk: false, mine: "3-0", mineOk: true },
        { h: "ARG", hf: "\ud83c\udde6\ud83c\uddf7", a: "MEX", af: "\ud83c\uddf2\ud83c\uddfd", status: "final", score: [1, 0], g: "1-0", gOk: true, mine: "2-1", mineOk: false },
        { h: "FRA", hf: "\ud83c\uddeb\ud83c\uddf7", a: "USA", af: "\ud83c\uddfa\ud83c\uddf8", status: "final", score: [3, 1], g: "3-1", gOk: true, mine: "3-0", mineOk: false },
        { h: "ESP", hf: "\ud83c\uddea\ud83c\uddf8", a: "CHI", af: "\ud83c\udde8\ud83c\uddf1", status: "final", score: [2, 1], g: "2-1", gOk: true, mine: "2-1", mineOk: true },
      ],
    }],
  },
  {
    ...
    rounds: [{
      ...
      matches: [
        { h: "BRA", hf: "\ud83c\udde7\ud83c\uddf7", a: "ARG", af: "\ud83c\udde6\ud83c\uddf7", status: "upcoming", time: "hoje 21:00", live: false, g: "2-1", mine: "2-2", featured: true },
        { h: "FRA", hf: "\ud83c\uddeb\ud83c\uddf7", a: "NED", af: "\ud83c\uddf3\ud83c\uddf1", status: "live", score: [0, 1], minute: "56'", g: "1-0", mine: "1-1" },
        { h: "GER", hf: "\ud83c\udde9\ud83c\uddea", a: "MEX", af: "\ud83c\uddf2\ud83c\uddfd", status: "final", score: [3, 1], g: "3-1", gOk: true, mine: "2-0", mineOk: false },
        { h: "POR", hf: "\ud83c\uddf5\ud83c\uddf9", a: "URU", af: "\ud83c\uddfa\ud83c\uddfe", status: "final", score: [2, 1], g: "1-1", gOk: false, mine: "1-1", mineOk: false },
      ],
    }],
  },
  {
    ...
    rounds: [{
      ...
      matches: [
        { h: "???", hf: "\u2753", a: "???", af: "\u2753", status: "locked", time: "05 jul 21:00" },
        { h: "???", hf: "\u2753", a: "???", af: "\u2753", status: "locked", time: "06 jul 21:00" },
      ],
    }],
  },
  {
    ...
    rounds: [{
      ...
      matches: [{ h: "???", hf: "\ud83c\udfc6", a: "???", af: "\ud83c\udfc6", status: "locked", time: "12 jul 17:00", multiplier: 5 }],
    }],
  },
];
...
function FixtureRow({ m, onOpen }) {
  ...
  return (
    <button onClick={() => !locked && onOpen?.(m)}
      disabled={locked}
      style={{
        textAlign: "left", width: "100%",
        background: m.featured ? "linear-gradient(90deg, rgba(255,201,101,0.1), var(--container-high))"
          : live ? "linear-gradient(90deg, rgba(255,110,132,0.08), var(--container-high))"
          : "var(--container-high)",
      ...
      <div style={{ textAlign: "center" }}>
        {live && <>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
            <span className="pulse-dot" style={{ width: 5, height: 5, background: "var(--error)", boxShadow: "0 0 8px var(--error)" }} />
            <span className="label-mono" style={{ fontSize: 8, color: "var(--error)" }}>LIVE</span>
          </div>
          <div className="num-mono" style={{ fontSize: 12, color: "var(--fg)", marginTop: 2 }}>{m.minute}</div>
        </>}
        {final && <div className="label-mono" style={{ fontSize: 9, color: "var(--muted)" }}>FINAL</div>}
        {upcoming && (
          <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: m.featured ? "var(--tertiary)" : "var(--muted)", fontWeight: 700, lineHeight: 1.3 }}>
            {m.time?.split(" ").map((s, i) => <div key={i}>{s}</div>)}
          </div>
        )}
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
            ...
            <span className="h-display" style={{ fontSize: 13, color: m.score && m.score[0] > m.score[1] ? "var(--fg)" : "var(--muted)" }}>{m.h}</span>
          </div>
          {(final || live) ? (
            <span className="h-display num-mono" style={{ fontSize: 16, color: live ? "var(--error)" : "var(--fg)", whiteSpace: "nowrap" }}>
              {m.score[0]} – {m.score[1]}
            </span>
          ) : (
            <span style={{ fontSize: 11, color: "var(--muted)", fontFamily: "var(--mono)" }}>vs</span>
          )}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6, marginTop: 3 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
            <span style={{ fontSize: 18 }}>{m.af}</span>
            <span className="h-display" style={{ fontSize: 13, color: m.score && m.score[1] > m.score[0] ? "var(--fg)" : "var(--muted)" }}>{m.a}</span>
          </div>
        </div>
        ...
        {!locked && (
          <div style={{
            ...
            paddingTop: 8, borderTop: "1px dashed var(--outline-variant)",
          }}>
            {m.g && (
              <span style={{
                display: "inline-flex", alignItems: "center", gap: 4,
                padding: "2px 7px", borderRadius: 6,
                background: m.gOk === true ? "rgba(79,243,37,0.15)" : m.gOk === false ? "rgba(255,110,132,0.15)" : "rgba(255,201,101,0.12)",
                color: m.gOk === true ? "var(--secondary)" : m.gOk === false ? "var(--error)" : "var(--tertiary)",
                fontFamily: "var(--mono)", fontSize: 10, fontWeight: 700,
              }}>
            ...
            {m.mine && (
              <span style={{
                display: "inline-flex", alignItems: "center", gap: 4,
                padding: "2px 7px", borderRadius: 6,
                background: m.mineOk === true ? "rgba(79,243,37,0.15)" : m.mineOk === false ? "rgba(255,110,132,0.15)" : "rgba(149,170,255,0.12)",
                color: m.mineOk === true ? "var(--secondary)" : m.mineOk === false ? "var(--error)" : "var(--primary)",
                fontFamily: "var(--mono)", fontSize: 10, fontWeight: 700,
              }}>
                Você {m.mine}
              </span>
    ...
    <div>
        ...
        {FIXTURES.map((p, i) => {
          ...
          return (
            <button key={i} onClick={() => setPhase(i)}
              ...
              {p.current && !on && <span className="pulse-dot" style={{ width: 5, height: 5 }} />}
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 12 }}>
          <div>
            <div className="h-display" style={{ fontSize: 22, color: ph.color }}>{ph.phase}</div>
            <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 2 }}>{ph.subtitle}</div>
          </div>
          {ph.current && (
            <span style={{
              display: "inline-flex", alignItems: "center", gap: 4,
...
function UAv({ nick, size = 28, ring = false }) {
  ...
  const initials = nick.replace(/[^a-z0-9]/gi, "").slice(0, 2).toUpperCase();
  return (
    <div style={{
      width: size, height: size, borderRadius: 999,
      padding: ring ? 2 : 0,
      background: ring ? "conic-gradient(from 180deg, var(--primary), var(--secondary), var(--tertiary))" : "transparent",
      flexShrink: 0,
    }}>
      <div style={{
        ...
        color: "white",
        display: "grid", placeItems: "center",
        fontFamily: "var(--headline)", fontSize: size * 0.36, fontWeight: 700,
      }}>{initials}</div>
    </div>
  );
}
...
function MemberStack({ members, size = 24, max = 4 }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center" }}>
      {visible.map((m, i) => (
        <div key={i} style={{ marginLeft: i === 0 ? 0 : -8, border: "2px solid var(--container)", borderRadius: 99, lineHeight: 0 }}>
          {m.isAI
      ...
      {more > 0 && (
        <div style={{
          ...
          color: "var(--muted)",
          display: "grid", placeItems: "center",
          fontFamily: "var(--mono)", fontSize: size * 0.35, fontWeight: 700,
        }}>+{more}</div>
      )}
    </div>
  );
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          {title && (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div className="h-display" style={{ fontSize: 17 }}>{title}</div>
...
function CreateGroupSheet({ open, onClose, onCreated }) {
  ...
  const [data, setData] = useState({
    name: "", emoji: "\ud83c\udfc6", color: "#95aaff",
    description: "", privacy: "private", includeGepeto: true,
    multiplierKO: 3, multiplierFinal: 5,
  });
  ...
  const submit = () => {
    const code = Array.from({ length: 6 }, () => "ABCDEFGHJKMNPQRSTUVWXYZ23456789".charAt(Math.floor(Math.random() * 31))).join("");
    setCreatedCode(code);
    setStep(3);
  };

  return (
    <Sheet open={open} onClose={handleClose} title={createdCode ? "Bolão criado!" : "Criar bolão"}>
      {/* Step indicator */}
      {!createdCode && (
      ...
      {step === 0 && !createdCode && (
        <div>
          <div className="label-mono" style={{ marginBottom: 8 }}>IDENTIDADE</div>
            ...
            <div style={{ flex: 1 }}>
              <input
                ...
                style={{
                  ...
                  color: "var(--fg)", fontFamily: "var(--headline)",
              ...
              }}>{data.name.length}/32</div>
            </div>
          </div>

          <div className="label-mono" style={{ marginBottom: 6 }}>EMOJI</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 4, marginBottom: 14 }}>
            {EMOJI_OPTIONS.map((e) => (
              <button key={e} onClick={() => setData({ ...data, emoji: e })}
                style={{
                  aspectRatio: "1/1", fontSize: 22,
                  background: data.emoji === e ? "var(--container-highest)" : "transparent",
          ...
          <div className="label-mono" style={{ marginBottom: 6 }}>COR DE ACENTO</div>
            ...
            {COLOR_OPTIONS.map((c) => (
              <button key={c} onClick={() => setData({ ...data, color: c })}
                style={{
                  ...
                  cursor: "pointer",
                }}><span style={{ display: "none" }}>{c}</span></button>
            ))}
          </div>

          <div className="label-mono" style={{ marginBottom: 6 }}>DESCRIÇÃO (opcional)</div>
          <textarea value={data.description}
            ...
            style={{
              ...
              fontSize: 12, minHeight: 60, resize: "vertical", outline: "none",
            }} />

          <button onClick={() => setStep(1)} disabled={!valid}
      ...
      {step === 1 && !createdCode && (
        <div>
          <div className="label-mono" style={{ marginBottom: 8 }}>QUEM PODE ENTRAR</div>
          {[
            { id: "private", icon: "lock", label: "Só quem tem o código", desc: "Você convida individualmente. Recomendado para amigos e família." },
            { id: "city", icon: "pin", label: "Público na minha cidade", desc: "Aparece em sugestões pra quem está em São Paulo." },
            { id: "open", icon: "users", label: "Aberto para todos", desc: "Qualquer um pode entrar. Indicado pra bolões grandes (50+)." },
          ].map((p) => (
            <button key={p.id} onClick={() => setData({ ...data, privacy: p.id })}
              style={{
                display: "flex", gap: 12, width: "100%",
                padding: 12, marginBottom: 8,
                background: data.privacy === p.id ? "rgba(149,170,255,0.08)" : "var(--container-high)",
              ...
              <div style={{
                ...
                background: data.privacy === p.id ? data.color : "var(--container-highest)",
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "var(--fg)" }}>{p.label}</div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3, lineHeight: 1.4 }}>{p.desc}</div>
              </div>
              {data.privacy === p.id && (
                <div style={{
                  width: 20, height: 20, borderRadius: 99,
                  background: data.color, color: "var(--bg)",
                  display: "grid", placeItems: "center", flexShrink: 0,
          ...
          <div style={{
            ...
            background: data.includeGepeto ? "rgba(255,201,101,0.08)" : "var(--container-high)",
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                Ele palpita em todos os jogos e provoca o grupo. Pode ser desligado depois.
              </div>
            </div>
            <div className={`switch ${data.includeGepeto ? "on" : ""}`}
              onClick={() => setData({ ...data, includeGepeto: !data.includeGepeto })}
              role="button" />
          </div>

          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            <button onClick={() => setStep(0)} className="btn btn-outline" style={{ paddingInline: 16 }}>
              <Icon name="chevronLeft" size={14} />
            </button>
      ...
      {step === 2 && !createdCode && (
        <div>
          <div className="label-mono" style={{ marginBottom: 8 }}>PONTUAÇÃO & MULTIPLICADORES</div>
          ...
          <div className="label-mono" style={{ marginBottom: 8 }}>RODADAS QUE VALEM MAIS</div>
          <SliderRow label="Mata-mata" value={data.multiplierKO}
            onChange={(v) => setData({ ...data, multiplierKO: v })}
            min={1} max={5} suffix="× pts" color={data.color} />
          <SliderRow label="Final" value={data.multiplierFinal}
            onChange={(v) => setData({ ...data, multiplierFinal: v })}
            min={1} max={10} suffix="× pts" color={data.color} />
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            <button onClick={() => setStep(1)} className="btn btn-outline" style={{ paddingInline: 16 }}>
              <Icon name="chevronLeft" size={14} />
            </button>
      ...
      {createdCode && (
        <CreatedSuccess data={data} code={createdCode} onShare={() => { handleClose(); }} />
      )}
    </Sheet>
  );
}
  ...
  return (
    <div style={{
      ...
      <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
        <span className="num-mono" style={{ fontSize: 12, fontWeight: 700 }}>{value}</span>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 12 }}>{label}</span>
        <span className="num-mono" style={{ fontSize: 13, fontWeight: 700, color }}>{value}{suffix}</span>
      </div>
  ...
  return (
    <div>
      <div style={{ textAlign: "center", padding: "10px 0 16px" }}>
        ...
        <div className="h-display" style={{ fontSize: 20, marginTop: 12 }}>{data.name}</div>
        <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>
          {data.includeGepeto ? "Você + Gepeto. Bora chamar a galera." : "Bora chamar a galera."}
        </div>
      </div>

      <div className="label-mono" style={{ marginBottom: 6 }}>CÓDIGO DE ENTRADA</div>
      <div style={{
        ...
        <div className="h-display num-mono" style={{
          fontSize: 32, letterSpacing: "0.25em", color: data.color,
        }}>{code}</div>
        <div className="label-mono" style={{ marginTop: 6 }}>{link}</div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        <button onClick={() => { setCopied(true); setTimeout(() => setCopied(false), 1500); }}
          className="btn btn-outline" style={{ fontSize: 12 }}>
          <Icon name={copied ? "check" : "share"} size={14} /> {copied ? "Copiado" : "Copiar link"}
        </button>
        <button className="btn btn-secondary" style={{ fontSize: 12, background: "#25d366", color: "white" }}>
          WhatsApp
        </button>
      </div>
...
function JoinGroupSheet({ open, onClose, onJoined }) {
  ...
  const reset = () => { setCode(""); setStep("input"); };
  const handleClose = () => { onClose(); setTimeout(reset, 200); };
  ...
  return (
    <Sheet open={open} onClose={handleClose} title="Entrar em um bolão">
      {step === "input" && (
        <div>
          <div style={{ textAlign: "center", padding: "10px 0 14px", fontSize: 13, color: "var(--muted)" }}>
          ...
          <div style={{
            ...
            <span>Ou abra um link <b style={{ color: "var(--primary)" }}>figfacil.com/b/XXXXXX</b> que alguém te mandou.</span>
          </div>
        </div>
      )}
        ...
        <div>
          <div style={{
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div style={{ flex: 1 }}>
                <div className="h-display" style={{ fontSize: 17 }}>{preview.name}</div>
            ...
            <div style={{
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, color: "var(--tertiary)" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setStep("input")} className="btn btn-outline" style={{ paddingInline: 16 }}>
              <Icon name="chevronLeft" size={14} />
            </button>
            <button onClick={() => { onJoined?.(); handleClose(); }} className="btn btn-primary" style={{ flex: 1 }}>
    ...
    <Sheet open={open} onClose={onClose} title="Convidar amigos">
      <div style={{ textAlign: "center", marginBottom: 14 }}>
        <div style={{
          ...
          background: "var(--container-high)",
          display: "inline-grid", placeItems: "center", fontSize: 28,
          border: `1px solid ${group.color}55`,
        }}>{group.emoji}</div>
        <div className="h-display" style={{ fontSize: 15, marginTop: 8 }}>{group.name}</div>
      </div>

      <div className="label-mono" style={{ marginBottom: 6 }}>CÓDIGO DE 6 LETRAS</div>
      <button onClick={() => copy(code, "code")} style={{
        ...
        <div className="h-display num-mono" style={{
          fontSize: 28, letterSpacing: "0.22em", color: group.color,
        }}>{code}</div>
        <span style={{
          fontSize: 11, color: "var(--muted)",
          display: "inline-flex", alignItems: "center", gap: 4,
      ...
      <div className="label-mono" style={{ marginBottom: 6 }}>OU LINK</div>
      <button onClick={() => copy(link, "link")} style={{
        ...
        {link}
        <Icon name={copied === "link" ? "check" : "share"} size={13} color="var(--muted)" />
      </button>

      <div className="label-mono" style={{ marginBottom: 8 }}>COMPARTILHAR</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
        <button className="btn btn-secondary" style={{
          background: "#25d366", color: "white", fontSize: 12,
        }}>WhatsApp</button>
      ...
      <div style={{
        ...
        <div className="label-mono" style={{ marginBottom: 4 }}>PRÉ-VISUALIZAÇÃO</div>
        {message}
      </div>

      <div style={{ marginTop: 16 }}>
        <div className="label-mono" style={{ marginBottom: 6 }}>PENDENTES (2)</div>
...
function PokeSheet({ open, onClose, target, match }) {
  ...
  const reset = () => { setPicked(0); setCustom(""); setSent(false); };
  const handleClose = () => { onClose(); setTimeout(reset, 200); };
    ...
    <Sheet open={open} onClose={handleClose} title={`Cutucar @${target}`}>
      {sent ? (
        <div style={{ textAlign: "center", padding: "30px 10px" }}>
          ...
          <div className="h-display" style={{ fontSize: 18, marginTop: 8 }}>Cutucada enviada!</div>
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 6 }}>
            @{target} recebeu uma notificação. Máximo 1 cutucada a cada 2h por pessoa.
          </div>
          <button onClick={handleClose} className="btn btn-primary" style={{ marginTop: 18, width: "100%" }}>
            Beleza
          </button>
        </div>
      ) : (
        <div>
          <div className="label-mono" style={{ marginBottom: 8 }}>ESCOLHA UM TOM</div>
          {templates.map((t, i) => (
            <button key={i} onClick={() => setPicked(i)}
              style={{
                display: "flex", gap: 10, width: "100%",
                padding: 12, marginBottom: 6,
                background: picked === i ? "rgba(149,170,255,0.08)" : "var(--container-high)",
          ...
          <div className="label-mono" style={{ marginTop: 14, marginBottom: 6 }}>OU ESCREVA</div>
          <textarea value={custom} onChange={(e) => setCustom(e.target.value)}
            ...
            style={{
              ...
              borderRadius: 10, padding: "10px 12px",
              color: "var(--fg)", fontFamily: "var(--body)",
              fontSize: 12, minHeight: 56, resize: "vertical", outline: "none",
            }} />
...
const SUGGESTED_GROUPS = [
  { id: "sptotal", name: "S\u00e3o Paulo Total", emoji: "\ud83c\udfdf\ufe0f", members: 247, online: 31, scope: "S\u00e3o Paulo" },
  { id: "bra2026", name: "Brasil Capit\u00e3o", emoji: "\ud83c\udfc6", members: 1842, online: 312, scope: "Brasil" },
  { id: "vamosgepeto", name: "Anti-Gepeto Coalition", emoji: "\ud83e\udd16", members: 89, online: 12, scope: "Comunidade" },
];

const GROUP_ACTIVITY = {
  trampo: [
    { type: "gepeto-shade", who: "gepeto", target: "rafa_dias", text: "Rafa palpitou 3-2 para Brasil. Eu cravei 2-1. Vamos ver quem entende de futebol.", when: "agora" },
    { type: "palpite", who: "ana.s", pick: "2-1 BRA", match: "BRA \u00d7 ARG", when: "2 min" },
    { type: "comment", who: "thiagomb", text: "Galera, alguma palpita\u00e7\u00e3o de FRA \u00d7 NED? T\u00f4 dividido", when: "8 min" },
    { type: "badge", who: "miltonfigueira", isMe: true, text: "bateu o Gepeto em GER \u00d7 MEX (+10 pts)", when: "1h" },
    { type: "palpite", who: "rafa_dias", pick: "3-2 BRA", match: "BRA \u00d7 ARG", when: "1h" },
    { type: "gepeto-self", who: "gepeto", text: "Cravei BRA 2-1 ARG. SHA-256: a4f8e1\u2026c92d. T\u00e1 selado. Boa sorte.", when: "3h" },
  ...
  return (
    <button onClick={() => onOpen(g.id)}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, paddingLeft: 4 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="h-display" style={{ fontSize: 15 }}>{g.name}</div>
          <div style={{ fontSize: 11, color: "var(--muted)", display: "flex", alignItems: "center", gap: 5, marginTop: 2 }}>
      ...
      <div style={{
        ...
        gap: 8, marginTop: 12, paddingLeft: 4,
      }}>
        <div style={{
          padding: "8px 8px", textAlign: "center",
          background: "var(--container-high)", borderRadius: 8,
          ...
          <div className="label-mono" style={{ fontSize: 8, marginTop: 2 }}>VOCÊ</div>
        </div>
        <div style={{
          padding: "8px 8px", textAlign: "center",
          background: "var(--container-high)", borderRadius: 8,
          ...
          <div className="num-mono" style={{ fontSize: 18, fontWeight: 700, color: "var(--tertiary)" }}>#{g.gepetoRank}</div>
          <div className="label-mono" style={{ fontSize: 8, marginTop: 2, display: "inline-flex", alignItems: "center", gap: 3 }}>
            <GepetoAvatar size={10} mood="neutral" glow={false} /> GEPETO
          </div>
        </div>
        <div style={{
          padding: "8px 8px", textAlign: "center",
          background: g.nextLocked > 0 ? "rgba(255,201,101,0.08)" : "var(--container-high)",
          borderRadius: 8,
          ...
          <div className="label-mono" style={{ fontSize: 8, marginTop: 2 }}>{g.nextLocked > 0 ? "FALTAM" : "OK"}</div>
        </div>
      </div>
    </button>
  );
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ marginBottom: 14 }}>
        <div className="h-display" style={{ fontSize: 22, marginBottom: 4 }}>Bolões</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
        <button onClick={onCreate}
          className="btn btn-primary" style={{ height: 44, flexDirection: "column", gap: 2, padding: "8px 6px" }}>
        ...
        <button onClick={onJoin}
          className="btn btn-outline" style={{ height: 44, padding: "8px 6px", fontSize: 12 }}>
          <Icon name="users" size={13} /> Entrar com código
        </button>
      </div>

      {/* My groups */}
      <div className="label-mono" style={{ marginBottom: 8 }}>MEUS BOLÕES · {MY_GROUPS.length}</div>
      {MY_GROUPS.map((g) => (
        <GroupCard key={g.id} g={g} onOpen={onOpen} />
      ))}

      {/* Suggested */}
      <div className="label-mono" style={{ marginTop: 18, marginBottom: 8 }}>SUGESTÕES</div>
      <div style={{ display: "grid", gap: 8 }}>
        {SUGGESTED_GROUPS.map((g) => (
          <div key={g.id} className="raised" style={{
            padding: 12,
            display: "flex", alignItems: "center", gap: 12,
          }}>
            <div style={{
            ...
            <button className="btn btn-outline" style={{ height: 30, paddingInline: 12, fontSize: 11 }}>
              Entrar
            </button>
          </div>
        ))}
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <button onClick={onBack} className="btn btn-ghost" style={{ height: 28, paddingInline: 4, fontSize: 12, marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div className="h-display" style={{ fontSize: 18 }}>{group.name}</div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }} />
          <button onClick={onInvite} className="btn btn-outline" style={{ height: 32, paddingInline: 12, fontSize: 11 }}>
      ...
      <div className="seg" style={{
        ...
        <button className={tab === "next" ? "active" : ""} onClick={() => setTab("next")}>Próximo</button>
      </div>
      ...
      {tab === "ranking" && (
        <div style={{ padding: 16 }}>
          ...
          <div className="raised" style={{
            ...
            <div className="label-mono" style={{ marginBottom: 8 }}>NESTE BOLÃO</div>
            <div style={{ display: "flex", alignItems: "center", gap: 14, justifyContent: "center" }}>
              <div style={{ textAlign: "center" }}>
                <UAv nick="miltonfigueira" size={36} />
                <div className="num-mono" style={{ fontSize: 22, fontWeight: 700, color: "var(--primary)", marginTop: 4 }}>#{meIdx + 1}</div>
              </div>
              <div className="h-display" style={{ fontSize: 16, color: meIdx < gepetoIdx ? "var(--secondary)" : "var(--error)" }}>
                {meIdx < gepetoIdx ? "BATENDO" : "PERDENDO"}
              </div>
              <div style={{ textAlign: "center" }}>
                <GepetoAvatar size={36} mood={meIdx < gepetoIdx ? "angry" : "smug"} glow={false} />
                <div className="num-mono" style={{ fontSize: 22, fontWeight: 700, color: "var(--tertiary)", marginTop: 4 }}>#{gepetoIdx + 1}</div>
              </div>
            </div>
          ...
          {/* Full leaderboard */}
          <div className="label-mono" style={{ marginBottom: 8 }}>RANKING GERAL</div>
          <div style={{ display: "grid", gap: 4 }}>
            {sorted.map((m, i) => {
              const rank = i + 1;
              const isPodium = rank <= 3;
              return (
                <div key={m.nick} style={{
                  display: "grid", gridTemplateColumns: "30px 32px 1fr auto",
                  alignItems: "center", gap: 10,
                  padding: "8px 10px",
                  background: m.isMe ? "rgba(149,170,255,0.1)"
                    : m.isAI ? "rgba(255,201,101,0.05)"
                    : "var(--container-high)",
                  ...
                  <div className="num-mono" style={{
                    fontSize: 14, fontWeight: 700,
                    color: rank === 1 ? "var(--tertiary)" : isPodium ? "var(--primary)" : "var(--muted)",
                    textAlign: "center",
                  }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      ...
                      {m.isAI && <span style={{
                        marginLeft: 6,
                        padding: "1px 5px", borderRadius: 4,
                        background: "rgba(255,201,101,0.2)", color: "var(--tertiary)",
                        fontFamily: "var(--headline)", fontSize: 7, fontWeight: 700, letterSpacing: "0.1em",
                      }}>IA</span>}
                    </div>
                  </div>
                  <div className="num-mono" style={{
                    fontSize: 14, fontWeight: 700,
                    color: m.isMe ? "var(--primary)" : m.isAI ? "var(--tertiary)" : "var(--fg)",
                  }}>{m.pts}pts</div>
                </div>
              );
            })}
        ...
        <div style={{ padding: 16 }}>
          {activity.length === 0 ? (
            <div style={{ padding: 30, textAlign: "center", color: "var(--muted)" }}>
              <div className="h-display" style={{ fontSize: 14, marginBottom: 4 }}>Silêncio sepulcral</div>
              <div style={{ fontSize: 12 }}>Quando alguém palpitar ou o Gepeto provocar, aparece aqui.</div>
            </div>
          ) : (
            <div style={{ display: "grid", gap: 8 }}>
          ...
          <div style={{
            ...
            <input placeholder="Manda uma resenha pro grupo..."
              style={{
                flex: 1, background: "transparent", border: 0, color: "var(--fg)",
                fontFamily: "var(--body)", fontSize: 13, outline: "none", padding: "0 6px",
              }} />
            <button className="btn btn-primary" style={{ height: 30, width: 30, padding: 0, borderRadius: 99 }}>
      ...
      {tab === "next" && (
        <NextMatchInGroup group={group} onPoke={onPoke} />
      )}
    </div>
  );
}
  ...
  if (a.type === "gepeto-shade" || a.type === "gepeto-self") {
    return (
      <div style={{
        ...
        <GepetoAvatar size={32} mood={a.type === "gepeto-shade" ? "smug" : "neutral"} glow={false} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--tertiary)" }}>Gepeto</span>
            {a.target && <>
              <span style={{ fontSize: 11, color: "var(--muted)" }}>provocou</span>
              <span style={{ fontSize: 12, fontWeight: 700 }}>@{a.target}</span>
            </>}
            <span style={{ marginLeft: "auto", fontSize: 10, color: "var(--muted)" }}>{a.when}</span>
          </div>
          <div style={{ fontSize: 12, color: "var(--fg)", marginTop: 4, lineHeight: 1.4, fontStyle: "italic" }}>
            “{a.text}”
          </div>
          <div style={{ display: "flex", gap: 12, marginTop: 6, fontSize: 11 }}>
            <button style={{ background: "transparent", border: 0, padding: 0, cursor: "pointer", color: "var(--muted)" }}>❤️ 4</button>
            <button style={{ background: "transparent", border: 0, padding: 0, cursor: "pointer", color: "var(--muted)" }}>✍️ Responder</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 2 }}>{a.match} · {a.when}</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{
        ...
        <div style={{ flex: 1, fontSize: 12 }}>
          <b style={{ color: a.isMe ? "var(--primary)" : "var(--fg)" }}>{a.isMe ? "Você" : `@${a.who}`}</b> {a.text}
        </div>
    ...
    return (
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--fg)", marginTop: 2, lineHeight: 1.4 }}>{a.text}</div>
        </div>
      </div>
    );
  }
  if (a.type === "join") {
    return (
      <div style={{
        ...
        <span><b>@{a.who}</b> {a.text} · {a.when}</span>
      </div>
    );
  }
  return null;
    ...
    <div style={{ padding: 16 }}>
      ...
      <div className="tcard" style={{ padding: "14px 14px", marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
          <div className="label-mono" style={{ color: "var(--tertiary)" }}>PRÓXIMO JOGO</div>
          <CountdownChip />
        </div>
        <div style={{
          display: "grid", gridTemplateColumns: "1fr auto 1fr",
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <div className="h-display" style={{ fontSize: 12, marginTop: 4 }}>{MATCH.home.code}</div>
          </div>
          <div className="h-display" style={{ fontSize: 18, color: "var(--muted)" }}>vs</div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 36, lineHeight: 1 }}>{MATCH.away.flag}</div>
            <div className="h-display" style={{ fontSize: 12, marginTop: 4 }}>{MATCH.away.code}</div>
          </div>
        </div>
      </div>

      {/* Predicted */}
      <div className="label-mono" style={{ marginBottom: 8 }}>{predicted.length} JÁ PALPITARAM</div>
      <div style={{ display: "grid", gap: 6, marginBottom: 14 }}>
        {predicted.map((m) => {
          ...
          return (
            <div key={m.nick} style={{
              ...
              alignItems: "center", gap: 10,
              padding: "8px 10px",
              background: m.isMe ? "rgba(149,170,255,0.08)"
                : m.isAI ? "rgba(255,201,101,0.06)"
                : "var(--container-high)",
              ...
              <span className="num-mono" style={{
                ...
                background: m.isAI ? "rgba(255,201,101,0.18)" : "var(--container-highest)",
      ...
      {pending.length > 0 && (
        <>
          <div className="label-mono" style={{ marginBottom: 8 }}>{pending.length} AINDA NÃO PALPITARAM</div>
          <div style={{ display: "grid", gap: 6, marginBottom: 14 }}>
            {pending.map((m) => (
              <div key={m.nick} style={{
                display: "grid", gridTemplateColumns: "32px 1fr auto",
                ...
                <button onClick={() => onPoke?.(m.nick)} className="btn btn-ghost" style={{ height: 26, paddingInline: 8, fontSize: 10 }}>
                  Cutucar
                </button>
              </div>
            ))}
      ...
      <div className="raised" style={{ padding: 14 }}>
        <div className="h-display" style={{ fontSize: 13, marginBottom: 10 }}>Consenso do grupo</div>
        <div style={{
          ...
          <div style={{
            width: "25%", display: "grid", placeItems: "center",
            background: "var(--outline-variant)",
            color: "var(--fg)", fontSize: 11, fontWeight: 700,
          }}>=25%</div>
          <div style={{
            width: "15%", display: "grid", placeItems: "center",
            background: "linear-gradient(135deg, #7ab8e0aa, #7ab8e055)",
            color: "white", fontSize: 11, fontWeight: 700,
          }}>15%</div>
        </div>
        <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 8, display: "inline-flex", gap: 6, alignItems: "center" }}>
...
function BoloesScreen() {
  ...
  const openPoke = (nick) => { setPokeTarget(nick); setSheet("poke"); };

  return (
    <>
      {group
        ? <BolaoDetail group={group} onBack={() => setOpenId(null)}
            onInvite={openInvite} onPoke={openPoke} />
        : <BoloesList onOpen={setOpenId}
      ...
      <InviteSheet open={sheet === "invite"} onClose={() => setSheet(null)} group={group} />
      <PokeSheet open={sheet === "poke"} onClose={() => setSheet(null)}
        target={pokeTarget} match={"BRA × ARG"} />
    </>
  );
}
    ...
    <div style={{ padding: 16 }}>
      ...
      <div className="tcard fade-up" style={{ marginBottom: 14, padding: 0, overflow: "hidden" }}>
        ...
        <div style={{ padding: "18px 16px 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
            <div className="label-mono" style={{ color: "var(--tertiary)" }}>PRÓXIMO DUELO</div>
            <CountdownChip />
          </div>
          <div style={{
            marginTop: 14,
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div className="h-display" style={{ fontSize: 13, marginTop: 4 }}>{m.home.code}</div>
            </div>
            <div className="h-display" style={{ fontSize: 18, color: "var(--muted)" }}>vs</div>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 40, lineHeight: 1 }}>{m.away.flag}</div>
              <div className="h-display" style={{ fontSize: 13, marginTop: 4 }}>{m.away.code}</div>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div className="num-mono" style={{ fontSize: 16, color: "var(--tertiary)", fontWeight: 700, lineHeight: 1, marginTop: 2 }}>74%</div>
              <div className="label-mono" style={{ fontSize: 8, marginTop: 2 }}>ACERTOS</div>
            </div>
          </div>
          <div className="h-display" style={{ fontSize: 18, color: "var(--muted)" }}>×</div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "flex-end" }}>
            <div style={{ minWidth: 0, textAlign: "right" }}>
              <div style={{ fontSize: 12, fontWeight: 700 }}>Você</div>
              <div className="num-mono" style={{ fontSize: 16, color: "var(--primary)", fontWeight: 700, lineHeight: 1, marginTop: 2 }}>68%</div>
              <div className="label-mono" style={{ fontSize: 8, marginTop: 2 }}>ACERTOS</div>
            </div>
            <div style={{
        ...
        <div style={{ padding: 14 }}>
          <button onClick={() => onGoTo("match")}
            className="btn btn-primary" style={{ width: "100%" }}>
            <Icon name="lightning" size={14} /> Ir para o duelo
          </button>
          <button onClick={() => onGoTo("fixtures")}
            className="btn btn-ghost" style={{ width: "100%", marginTop: 4, height: 36, fontSize: 12 }}>
      ...
      <div className="raised fade-up" style={{ padding: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <button onClick={() => onGoTo("boloes")} style={{
        ...
        <div style={{ display: "grid", gap: 6 }}>
          {MY_GROUPS.slice(0, 2).map((g) => (
            <button key={g.id} onClick={() => onGoTo("boloes")}
              style={{
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: "var(--muted)" }}>#{g.myRank} de {g.totalRanks} · Gepeto #{g.gepetoRank}</div>
              </div>
              {g.nextLocked > 0 && (
                <span style={{
      ...
      <div className="raised fade-up" style={{ padding: 14, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div className="h-display" style={{ fontSize: 13 }}>Capítulo 3 · Quartas</div>
          <button onClick={() => onGoTo("weekly")} style={{
            background: "transparent", border: 0, padding: 0, cursor: "pointer",
            color: "var(--primary)", fontSize: 11, fontWeight: 700,
            display: "inline-flex", alignItems: "center", gap: 4,
          }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
          <div style={{
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
              ...
              <span className="label-mono" style={{ fontSize: 8 }}>GEPETO</span>
            </div>
            <div className="num-mono" style={{ fontSize: 28, color: "var(--tertiary)", fontWeight: 700, lineHeight: 1 }}>
          ...
          <div style={{
            ...
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
              ...
              <span className="label-mono" style={{ fontSize: 8 }}>HUMANOS</span>
            </div>
            <div className="num-mono" style={{ fontSize: 28, color: "var(--primary)", fontWeight: 700, lineHeight: 1 }}>
      ...
      <div style={{
        ...
        <button onClick={() => onGoTo("ranking")} style={{
          background: "transparent", border: 0, padding: 0, cursor: "pointer",
...
function RankingScreen() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div className="tcard fade-up" style={{ padding: 0, marginBottom: 14, overflow: "hidden" }}>
        <div className="tcard-bg" style={{
          background: "linear-gradient(135deg, rgba(149,170,255,0.18), rgba(13,19,35,1) 70%)",
        }} />
        <div style={{ padding: "16px 16px 0" }}>
          <div className="label-mono" style={{ color: "var(--primary)" }}>VOCÊ × GEPETO</div>
          <div className="h-display" style={{ fontSize: 18, marginTop: 4 }}>O dossiê da rivalidade</div>
        </div>
        <div style={{
          display: "grid", gridTemplateColumns: "1fr auto 1fr",
          alignItems: "center", gap: 14,
          ...
          <div style={{ textAlign: "center" }}>
            <div style={{
              ...
              margin: "0 auto",
              display: "grid", placeItems: "center",
              fontFamily: "var(--headline)", fontSize: 14, fontWeight: 700,
            }}>MI</div>
            <div className="h-display num-mono" style={{ fontSize: 38, marginTop: 6, color: "var(--primary)", letterSpacing: "-0.02em" }}>8</div>
            <div className="label-mono" style={{ fontSize: 8 }}>VITÓRIAS</div>
          </div>
          <div className="h-display" style={{ fontSize: 18, color: "var(--muted)" }}>×</div>
          <div style={{ textAlign: "center" }}>
            <GepetoAvatar size={38} mood="angry" glow={false} />
            <div className="h-display num-mono" style={{ fontSize: 38, marginTop: 6, color: "var(--tertiary)", letterSpacing: "-0.02em" }}>8</div>
            <div className="label-mono" style={{ fontSize: 8 }}>VITÓRIAS</div>
          </div>
        </div>
        <div style={{
          ...
          Tá <b style={{ color: "var(--tertiary)" }}>empatado</b>. A próxima rodada desempata.
        </div>
      </div>

      {/* Scope tabs */}
      <div className="seg" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", width: "100%", marginBottom: 12 }}>
        <button className={scope === "week" ? "active" : ""} onClick={() => setScope("week")}>Semana</button>
        <button className={scope === "cup" ? "active" : ""} onClick={() => setScope("cup")}>Copa</button>
        <button className={scope === "city" ? "active" : ""} onClick={() => setScope("city")}>Cidade</button>
      </div>

      {/* Leaderboard */}
      <div className="raised" style={{ padding: 12, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
          <div className="h-display" style={{ fontSize: 13 }}>Quem bateu mais o Gepeto</div>
          <span className="label-mono" style={{ fontSize: 9 }}>{scope === "week" ? "SEMANA 3" : scope === "cup" ? "COPA 2026" : "SÃO PAULO"}</span>
        </div>
        <div style={{ display: "grid", gap: 6 }}>
          {RANKING_DATA.map((u) => (
            <div key={u.nick} style={{
              ...
              <div className="num-mono" style={{
                fontSize: 14, fontWeight: 700,
                color: u.rank === 1 ? "var(--tertiary)" : u.rank <= 3 ? "var(--primary)" : "var(--muted)",
              }}>#{u.rank}</div>
              <div style={{
                width: 28, height: 28, borderRadius: 99,
                background: `hsl(${u.nick.charCodeAt(0) * 13 % 360} 40% 35%)`,
              ...
              <div style={{ minWidth: 0 }}>
                ...
                {u.streak && (
                  <div style={{ fontSize: 10, color: "var(--muted)", display: "inline-flex", alignItems: "center", gap: 3 }}>
                    <Icon name="flame" size={9} color="var(--tertiary)" /> {u.streak} streak
                  </div>
                )}
              </div>
              <div className="num-mono" style={{ fontSize: 13, color: "var(--secondary)", fontWeight: 700, whiteSpace: "nowrap" }}>
                {u.wins}×
              </div>
              <div className="num-mono" style={{ fontSize: 13, color: "var(--fg)", fontWeight: 700, whiteSpace: "nowrap" }}>
                {u.pts}pts
              </div>
            </div>
          ))}
      ...
      <div className="raised" style={{ padding: 14, marginBottom: 14 }}>
        <div className="h-display" style={{ fontSize: 13, marginBottom: 10 }}>Sua história contra o Gepeto</div>
        <div style={{
          ...
          {Array.from({ length: 16 }).map((_, i) => {
            ...
            return (
              <div key={i} title={`Rodada ${i + 1}`} style={{
                aspectRatio: "1/1",
                borderRadius: 6,
                background: won ? "linear-gradient(160deg, rgba(79,243,37,0.3), rgba(255,201,101,0.15))"
                  : tie ? "var(--container-highest)"
        ...
        <div style={{
          ...
          <span><b style={{ color: "var(--secondary)" }}>✓</b> bateu</span>
          <span><b style={{ color: "var(--muted)" }}>=</b> empate</span>
          <span><b style={{ color: "var(--error)" }}>✗</b> perdeu</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div style={{ padding: "0 16px 30px" }}>
        {showVerdict && (() => {
          const userExact = USER_PREDICTION.home === MATCH_RESULT.home && USER_PREDICTION.away === MATCH_RESULT.away;
          const gepetoExact = gepetoCorrect(MATCH.gepeto);
          const userWinner = (USER_PREDICTION.home > USER_PREDICTION.away) === (MATCH_RESULT.home > MATCH_RESULT.away);
          const userPts = userExact ? 25 : userWinner ? 10 : 0;
          const gepetoPts = gepetoExact ? 25 : 10;
        ...
        <YourPalpite match={MATCH} state={state}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", lineHeight: 1.05 }}>
            <span className="h-display" style={{ fontSize: 13, whiteSpace: "nowrap" }}>Gepeto</span>
            <span className="label-mono" style={{ fontSize: 8, lineHeight: 1, whiteSpace: "nowrap" }}>HUMANO × IA</span>
          </div>
        </div>
        <div style={{ width: 50 }} />
      </div>
      <div className="seg" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", width: "100%" }}>
        <button className={view === "home" ? "active" : ""} onClick={() => onView("home")} style={{ padding: "8px 4px", fontSize: 11 }}>Hub</button>
        <button className={view === "fixtures" ? "active" : ""} onClick={() => onView("fixtures")} style={{ padding: "8px 4px", fontSize: 11 }}>Jogos</button>
        <button className={view === "boloes" ? "active" : ""} onClick={() => onView("boloes")} style={{ padding: "8px 4px", fontSize: 11 }}>Bolões</button>
        <button className={view === "weekly" ? "active" : ""} onClick={() => onView("weekly")} style={{ padding: "8px 4px", fontSize: 11 }}>Capítulo</button>
        <button className={view === "ranking" ? "active" : ""} onClick={() => onView("ranking")} style={{ padding: "8px 4px", fontSize: 11 }}>Vs IA</button>
      </div>
    </div>
  );
}
...
function App() {
  ...
  return (
    <>
      <div className="device" data-screen-label={
        view === "home" ? "Gepeto · Hub" :
        view === "match" ? "Gepeto · Partida" :
        view === "weekly" ? "Gepeto · Capítulo semanal" :
          ...
          {view === "match" && (
            <>
              <div style={{ padding: "8px 16px 0" }}>
                <button onClick={() => setView("fixtures")}
                  className="btn btn-ghost" style={{ height: 28, paddingInline: 4, fontSize: 12 }}>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakSection label="Extras">
          <TweakToggle label="Streak vs Gepeto" value={tweaks.showStreak} onChange={(v) => setTweak("showStreak", v)} />
        </TweakSection>
      </TweaksPanel>
    </>
  );
